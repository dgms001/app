...
if (mapA == 993001100) { //限制開啟地圖
    ...
    text = ""
    text += "\t" + Icon[17][1] + "#fs20##fc0xFF00caf2#自選系列" + Icon[17][1] + "#r\r\n";
    ...
    text += "\t\t\t  #r強化道具\r\n"
    for (var i = 0; i <= 2; i++) {
        text += "#b#fs11##L" + i + "##v" + itemList[i] + "##g#z" + itemList[i] + "##l\r\n\r\n";
...
if (player.getFreeSlots(1) < YN) {
    npc.say("裝備欄不足，請清理");
} else if (!player.hasItem(boxId, YN * 10)) {
    npc.say("卡洛斯的意志不足" + YN * 10 + "");
} else if (QSS >= YN) {
    player.gainItem(itemList[selected], YN);
    player.loseItem(boxId, -10 * YN);
