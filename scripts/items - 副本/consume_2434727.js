...
switch (selection) {
    ...
    case 7: //初級會員福利
        if (player.getEventValue("每日福利初級會員") == 0) {
            addHyPay(35);
            player.gainItem(4001715, 1); //億
            player.gainItem(2023380, 2); //經驗卡
            player.gainItem(2023744, 5); //領路者
            player.gainItem(2023745, 5); //領路者
            player.gainItem(5060048, 1); //黃金蘋果
            player.gainItem(2022709, 2); //雙爆
            player.addEventValue("每日福利初級會員", 1, 1);
            npc.broadcastWeatherEffectNotice(207, "[初級會員] 玩家 " + player.getName() + " 領取了會員每日福利", 3000); //35[特效 1-???] 1000[1秒]
        } else {
            npc.say("您今天已經領取過了");
        }
        break;
    case 8: //高級會員福利
        if (player.getEventValue("每日福利高級會員") == 0) {
            ...
            player.gainItem(4001715, 2); //億
            player.gainItem(2023380, 4); //經驗卡
            player.gainItem(2023744, 10); //領路者
            player.gainItem(2023745, 10); //領路者
            player.gainItem(5060048, 30); //黃金蘋果
            player.gainItem(2022709, 4); //雙爆
            player.addEventValue("每日福利高級會員", 1, 1);
            npc.broadcastWeatherEffectNotice(208, "[高級會員] 玩家 " + player.getName() + " 領取了會員每日福利", 8000); //35[特效 1-???] 1000[1秒]
        } else {
            npc.say("您今天已經領取過了");
        }
        break;
