...
sel += "#b首斗禮包\r\n";
sel += "#b贊助金額達到" + condition + "元可領取首斗禮包\r\n";
sel += "#b累計消費達1000無法領取,需通過贊助達1000#n\r\n";
sel += "#b當前贊助:" + getHyPay(2) + "\r\n";


if (getHyPay(2) >= 1000) {
    sel += "#b#L" + 0 + "# [#r可領取#d]領取首充禮包 #l\r\n\r\n";
} else {
    sel += "#r#L" + 0 + "# [未完成]首充禮包 #l\r\n\r\n";
    ...
    if (YN == 1) {
        ...
        if (player.getEventValue("首充禮包") == 1) {
            ...
            if (player.getFreeSlots(1) < rewardlist.length || player.getFreeSlots(2) < rewardlist.length || player.getFreeSlots(3) < rewardlist.length || player.getFreeSlots(4) < rewardlist.length || player.getFreeSlots(5) < rewardlist.length) {
                ...
                nnpc.broadcastWeatherEffectNotice(237, "首衝禮包:感謝玩家" + player.getName() + "為本服做出了伺候器貢獻,這是給你送上的首衝福利禮包", 10000);
...
function getHyPay(type) {
    ...
    if (push.size() > 0) {
        ...
        if (type == 1) {
            ...
            var pay = result.get("paihangbang2");
        }
    } else {
