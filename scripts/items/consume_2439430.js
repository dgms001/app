...
if (mapA == 993001100) { //限制開啟地圖
    ...
    player.gainItem(2049323, 5);
    player.gainItem(4009454, 500);
    player.gainItem(4032053, 200);
    player.gainItem(4000444, 3000);
    player.loseItem(npc.getItemId())
    player.addEventValue("每日福利檢測", 1, 1);
    npc.broadcastPlayerNotice(37, "玩家" + player.getName() + " 獲得了每日0檔福利,當天不領隔天箱子自動刪除");
    if (getvalue() == 0) {
        insertitems(MAC, 1);
    } else {
        updateitems(MAC, 1);
...
function delvalue() {
    var sql = "delete from dgms_AllworldLog where ID > 0";
    player.customSqlUpdate(sql);
