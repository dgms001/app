...
if (mapA == 993001100) { //限制開啟地圖
    ...
    if (player.getEventValue("初級會員開通") == 0 && player.getEventValue("高級會員開通") == 0&& player.getEventValue("永久會員開通") == 0) { //沒有天數清空log和盒子
    ...
    if (player.getEventValue("高級會員開通") >= 1) {
        ...
        txt += "#fs16##e#b當前月費會員剩餘天數:[#r" + player.getEventValue("高級會員開通") + "#b]天\r\n";
...
switch (selection) {
    ...
    case 7: //初級會員福利
        if (player.getEventValue("每日福利初級會員") == 0) {
            ...
            npc.broadcastWeatherEffectNotice(207, "[理財周權] 玩家 " + player.getName() + " 領取了會員每日福利", 3000); //35[特效 1-???] 1000[1秒]
        } else {
            npc.say("您今天已經領取過了");
        }
        break;
    case 8: //高級會員福利
        if (player.getEventValue("每日福利高級會員") == 0) {
            ...
            npc.broadcastWeatherEffectNotice(208, "[理財月權] 玩家 " + player.getName() + " 領取了會員每日福利", 8000); //35[特效 1-???] 1000[1秒]
        } else {
            npc.say("您今天已經領取過了");
        }
        break;
    case 9: //一鍵滿怪怪卡
        if (player.getEventValue("每日福利永久會員") == 0) {
            ...
            npc.broadcastWeatherEffectNotice(208, "[理財永久權] 玩家 " + player.getName() + " 領取了會員每日福利", 8000); //35[特效 1-???] 1000[1秒]
        } else {
            npc.say("您今天已經領取過了");
        }
        break;
