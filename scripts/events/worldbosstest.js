...
function newopen() {
    if (parseInt(event.getVariable("state")) == 0) {
		...
		map_2.reset();
		map_2.showTimer(15*60);
		endTime = new Date().getTime() + 15 * 1000;
		event.setVariable("endTime", endTime);
		event.startTimer("scheduledTimeout",15*1000);
		//event.broadcastWeatherEffectNotice(135,"【世界BOSS副本即將開始,請大家做好準備！】",15000);
		   }
}
...
function kick() {
		...
		players.showSystemMessage("測試顯示");
		}
