...
let BOSS_LOG = "boss_blackmage_hard";
let party;
let members;
let endTime;
...
let start;
//let totalTime=0;
let bossHp = 800000000000000 + 800000000000000 + 800000000000000 + 800000000000000;//1+2+3 階段123 要和map裡面一致 如果有4個階段 3=3+4 不然血量計算會bug
var bossid = "黑魔法師[困難]";
let killPlayer = 0;
	...
	for (var i = 0; i < damList.length; i++) {
		...
		if (i == 0) { //第一名
			if (party.getMembersCount() > 1) {
				...
				event.broadcastPlayerNotice(49, "【" + bossid + "】 " + player.getName() + "隊伍的通關時間  " + mm + " 分 " + ss + " 秒 ");
			} else {
				var sql = "insert into dgms_bossinfo(bossName, playersName, `time`) values('" + bossid + "','" + player.getName() + "','" + totalTime + "')";
				player.customSqlInsert(sql);
				mm = Math.floor(parseFloat(totalTime) / 60);
