...
let EXIT_MAP = 271040000;
let mobId = 8850011; //增加
let bossHp = 202500000000; //增加
var bossid = "西格諾斯[困難]";
...
let end;
let totalTime = 0;
let BOSS_LOG = "boss_shinas_HARD";
var bossname = "西格諾斯[困難]";
let DEATH_COUNT = 5;
var tupoTotal = 0;
let members;
function init(attachment) {
    ...
    map.spawnMob(mob, -363, 100);
    mob.changeBaseHp(bossHp);
    map.blowWeather(5120043, "好久沒看到有人來了，但是也沒看到有人可以回去。");
...
function mobDied(mob) {
    if (mobId == mob.getDataId()) {
        ...
        for (var i = 0; i < damList.length; i++) {
            ...
            /*
            if (player.getEventValue("西格諾斯困難全員獎勵") >= 1) { }
            else {
                player.addEventValue("西格諾斯困難全員獎勵", 1, 1);
