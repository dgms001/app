...
function init(attachment) {
		...
		map_1.blowWeatherEffectNotice("擊敗了海上龍王,請找【萊文'達尼爾拉】領取獎勵。 10s後開啟", 134, 10000);
...
function check(){
	...
	if(stage<18){
		if(stage == check && map_1.getEventMobCount()==0){
			//說明10秒都沒有動過狀態，並且地圖剩余怪物為0 是卡死了龍王副本.
    ...
    switch (stage) {
        case 0:
            if (!"clear".equals(event.getVariable("stage" + stage))) {
                for (var i = 0; i < 15; i++) {
                    var mob = map_1.makeMob(9390868);
					mob.changeBaseHp(103000000);//8E
        ...
        case 3:
            if (!"clear".equals(event.getVariable("stage" + stage))) {
                var mob = map_1.makeMob(9390802); /* 暗礁 */
				mob.changeBaseHp(9000000000);//90E
        ...
        case 4:
            if (!"clear".equals(event.getVariable("stage" + stage))) {
                for (var i = 0; i < 15; i++) {
                    var mob = map_1.makeMob(9390824);
					mob.changeBaseHp(1030000000);//10E
                    map_1.spawnMob(mob, 220 - i * 35, 199);
                }
                map_1.blowWeatherEffectNotice("海螺姑娘的聲音真是動聽。我的意識漸漸變得模糊。", 133, 5000);
                stageClear(stage);
            }
            break;
        case 5:
            if (!"clear".equals(event.getVariable("stage" + stage))) {
                ...
                map_1.blowWeatherEffectNotice("小心。船長。雖然海螺姑娘的外表美麗。但是他們十分兇殘。", 133, 5000);
                stageClear(stage);
            }
            break;
        case 6:
        ...
        case 9:
            if (!"clear".equals(event.getVariable("stage" + stage))) {
                for (var i = 0; i < 20; i++) {
                    var mob = map_1.makeMob(9390824); /* 海螺姑娘 */
					 mob.changeBaseHp(2000000000);//20E
        ...
        case 11:
            if (!"clear".equals(event.getVariable("stage" + stage))) {
				...
				mob.changeBaseHp(15000000000);//150E
        ...
        case 12:
            if (!"clear".equals(event.getVariable("stage" + stage))) {
				...
				mob.changeBaseHp(20000000000);//200E
        ...
        case 16:
            if (!"clear".equals(event.getVariable("stage" + stage))) {
				...
				mob.changeBaseHp(30000000000);//300E
                map_1.spawnMob(mob, 396, 484);
                var mob = map_1.makeMob(9390835);//深海巨妖的觸手
				mob.changeBaseHp(3000000000);//50E
        ...
        case 17:
            if (!"clear".equals(event.getVariable("stage" + stage))) {
				...
				mob.changeBaseHp(30000000000);//500E
                map_1.spawnMob(mob,396, 484);
                var mob = map_1.makeMob(9390835);//深海巨妖的觸手
				mob.changeBaseHp(3000000000);//50E
                map_1.spawnMob(mob, 63, 455);
                map_1.blowWeatherEffectNotice("船長,請把它擊退吧。拜托你了。 就全交給船長了。", 133, 5000);
                stageClear(stage);
            }
            break;
        case 18:
			if (!"clear".equals(event.getVariable("stage" + stage)) ) {
				...
				mob.changeBaseHp(800000000000);//800E
                map_1.spawnMob(mob,690, 454);
...
//所有註冊在事件裡面的怪物都死亡後執行 
	////java.lang.System.out.println("龍王：剩余怪物"+map_1.getEventMobCount());
	if(map_1.getEventMobCount()==0){
		...
		switch (stage) {
			...
			case 12:
				...
				break;
			case 13:
				map_1.blowWeatherEffectNotice("區區水龍......這無非是證明了飛禽走獸是無法戰勝人類的。", 134, 5000);
				break;
			case 14:
			...
			case 18:
				...
				break;
			case 19:
				map_1.blowWeatherEffectNotice("真棒 ! ! ! 船長. 這趟有驚無險 快點擊 萊文 領你的航海獎品吧 ! 期待與船長下回的航海之旅。", 134, 5000);
				break;
