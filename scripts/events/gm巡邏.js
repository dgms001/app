...
let start;
let endTime;
let everyTime = 1*5* 1000;//功能時效 大於副本時間+進場時間等等...
...
// npc.say("");
	...
	event.setVariable("members", members);
		for (let i = 0; i < members.length; i++) {
				...
				members[i].showSystemMessage("個人通知｜開啟了 "+LOG+"");
				members[i].showSystemMessage("個人通知｜清除生怪功能,請在5秒內擊殺怪物恢復生怪功能");
...
function mobDied(mob) {
	...
	map.setNoSpawn​(false);
		for (let i = 0; i < members.length; i++) {
				members[i].showSystemMessage("個人通知｜恢復生怪功能了");
		}
...
function kick() {//時間到
	for (let i = 0; i < members.length; i++) {
			...
			members[i].showSystemMessage("kick|gm巡邏事件結束");
	}
	event.destroyEvent();
}
