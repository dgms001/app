...
let MAP_ID = 865020013;//0~1000 , 210
let BOSS_LOG = "改版圖騰副本2";
let level = 0;
let mustLevel = 1;
let totalTime = 0;
let party;
...
function init(attachment) {
	...
	for (let i = 0; i < members.length; i++) {
		if (members[i].getEventValue("改版圖騰副本2") >= 3) {
...
function mobDied(mob) {
	...
	if (mob.getDataId() == 8850100 || mob.getDataId() == 8850101 || mob.getDataId() == 8850102 || mob.getDataId() == 8850103 || mob.getDataId() == 8850104) {
		...
		if (DROPS < 25000) {
			for (let i = 0; i < members.length; i++) {
				members[i].runScript("改版圖騰強化4");
			}
		}
		if (DROPS < 12500) {
			for (let i = 0; i < members.length; i++) {
				members[i].runScript("改版圖騰強化5");
			}
		}
		if (DROPS < 7500) {
			for (let i = 0; i < members.length; i++) {
				members[i].runScript("改版圖騰強化6");
			}
		}
