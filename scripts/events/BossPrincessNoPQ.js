 ...
 * qq:381991414
 */
let MAP_ID = 811000500;
let EXIT_MAP = 811000999;
let mobId = 9450022; //增加
let bossHp = 1020000000000; //增加
var bossid = "濃姬[普通]";
...
function init(attachment) {
    ...
    for (let i = 0; i < members.length; i++) {
        ...
        if (members[i].getEventValue(BOSS_LOG) > 2) {
            members[i].changeMap(EXIT_MAP);
...
function mobDied(mob) {
    if (mobId == mob.getDataId()) {
        ...
        for (var i = 0; i < damList.length; i++) {
            ...
            /*
            if (player.getEventValue("濃姬普通全員獎勵") >= 1) { }
            else {
                player.addEventValue("濃姬普通全員獎勵", 1, 1);
                player.gainItem(4032053, 100);
                player.gainItem(4001713, 50);
                player.gainItem(2630594, 70);
