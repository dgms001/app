...
let DEATH_COUNT = 5;
let BOSS_LOG = "boss_demian";
let party;
let members;
let endTime;
let start;
let totalTime = 0;
let bossHp = 800000000000 + 360000000000;
var bossid = "戴米安[普通]";
let killPlayer = 0;
var bossname = "戴米安[普通]";
var tupoTotal = 0;
        ...
        for (var i = 0; i < damList.length; i++) {
                ...
                if (i == 0) { //第一名
                        if (party.getMembersCount() > 1) {
                                ...
                                //event.broadcastPlayerNotice(49, "【" + bossid + "】 通關時間  " + mm + " 分 " + ss + " 秒 ");
                        } else {
                                var sql = "insert into dgms_bossinfo(bossName, playersName, `time`) values('" + bossid + "','" + player.getName() + "','" + totalTime + "')";
                                player.customSqlInsert(sql);
                                mm = Math.floor(parseFloat(totalTime) / 60);
