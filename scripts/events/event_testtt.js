...
function init(attachment) {
    ...
    //event.startTimer("destroyEvent", 1 * 60 * 1000);//30分鐘人數不滿就銷毀EVENT。
...
function playerChangedMap(player, destination) {
    if (destination.getId() != EVENT_FIELD) {
        ...
        //broadcastEventNotice(9, player.getName() + "加入了跑旗賽！目前 " + members.length + "" + (members.length <= minPlayer?"/" + minPlayer + "":"") + "人。");
		player.dropMessage(9, player.getName() + "加入了跑旗賽！目前 " + members.length + "" + (members.length <= minPlayer?"/" + minPlayer + "":"") + "人。");
    }
...
function timerExpired(key) {
    switch (key) {
        ...
        case "checkMembers":
            if (members.length >= minPlayer) {
                if (event.getVariable("readyStart") == null) {
                    ...
                    //broadcastEventNotice(-2, "即將開始跑旗賽，還有1分鐘時間等待其他玩家。");
                    //broadcastEventNotice(9, "即將開始跑旗賽，還有1分鐘時間等待其他玩家。");
                    //event.broadcastPlayerNotice(1, "[跑旗賽]還有1分鐘，跑旗賽就要開始了，沒有集合的小伙伴抓緊啦。");
					
        player.dropMessage(9, "即將開始跑旗賽，還有1分鐘時間等待其他玩家。");
                    event.startTimer("startEvent", 60 * 1000); //最後等待1分鐘開始活動。
                    event.getMap(EXIT_MAP).showTimer(60);
            ...
            if (members.length != lastMemberLength) {
                //broadcastEventNotice(-2, "正在等待其他玩家，目前 " + members.length + "/" + minPlayer + "人。");
				 player.dropMessage(9, "正在等待其他玩家，目前 " + members.length + "/" + minPlayer + "人。");
        ...
        case "startEvent":
            ...
            //broadcastEventNotice(-2, "跑旗賽開始啦！");
			
				 player.dropMessage(9, "跑旗賽開始啦！");
            //broadcastEventNotice(9, "跑旗賽開始啦！");
        ...
        case "prepareEndEvent":
            if (event.getVariable("prepareEndEvent") != true) {
                ...
                //broadcastEventNotice(-2, "距離跑旗賽結束還有60秒，抓緊時間！");
				
				 player.dropMessage(9, "距離跑旗賽結束還有60秒，抓緊時間！");
                //broadcastEventNotice(9, "距離跑旗賽結束還有60秒，抓緊時間！");
            }
            break;
        case "endEvent":
            ...
            event.setVariable("started", false);
            //event.broadcastPlayerNotice(1, "[跑旗賽]已經結束，歡迎下次參加。");
				 player.dropMessage(9, "[跑旗賽]已經結束，歡迎下次參加。");
            event.destroyEvent();
