...
var mobs = [9101078]
let allHp = 3000000000000000;//血量後期開到250即可
...
function init(attachment) {
    ...
    for (let dd = 0; dd < members.length; dd++) {
        if (members[dd].getEventValue("世界BOSS進入次數") >= 5) {
            members[dd].changeMap(103000000);
            members[dd].showSystemMessage("進入次數已達5次");
        }
    ...
    if (mobs.indexOf(mob.getDataId()) != -1) {
        ...
        if (killmobs == 1) {
            ...
            if (mob.getDataId() == 9101078 && rewardType) {
                ...
                FIELDMAP.scriptProgressMessage("擊殺成功!!!");
                FIELDMAP.broadcastEventNotice("擊殺成功!!!");
