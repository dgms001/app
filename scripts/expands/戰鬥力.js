...
var d1 = "#fUI/UIWindow2.img/Arbeit/298#" //排行
...
function start() {
    ...
    sql += "," + totalScore + " ) ";
    var Sql = "INSERT INTO zz_xr_totalscore VALUE (" + player.getId() + ",'" + player.getName() + "','" + jobType4(player.getJob()) + "',0 ," + totalScore + ")";
...
function equipmentRanking() {
    ...
    //查詢出對應的人物
    var selected = npc.askMenu(characterEquipInfo());
...
function characterEquipInfo(characterid) {
    ...
    for (var i = 0; i < resultList.size(); i++) {
        ...
        text = "[#r" + result.get("name") + "#k]：裝備1評分為：" + (result.get("總評分")) + "#k\r\n";
        ...
        text += writeText("圖騰2", result) + writeText("圖騰3", result) + writeText("神祕1", result) + writeText("神祕2", result) + "\r\n";
        text += writeText("神祕3", result) + writeText("神祕4", result) + writeText("神祕5", result) + writeText("神祕6", result) + "\r\n";
...
function equipmentRankingText() {
    ...
    var resultList = player.customSqlResult(sql);
    var text = "" + nbsp10("職業") + "    " + nbsp10("角色名") + "    " + "評分" + "\r\n";
    for (var i = 0; i < resultList.size(); i++) {
        var result = resultList.get(i);
        if (result == null) {
            break;
        ...
        if (i < 30) {
            text += "#fUI/UIWindow8/HundredShooting/number/" + (i + 1) + "#" + nbsp10(result.get("jobname")) + "" + nbsp10(result.get("name")) + "     " + result.get("總評分") + "#l\r\n";
        } else {
            text += "" + numberString(i + 1) + "" + nbsp10(result.get("jobname")) + "" + nbsp10(result.get("name")) + "     " + result.get("總評分") + "#l\r\n";
        }
    }
    return text;
}
...
function addInventoryitems(id) {
    //var sql = "INSERT INTO zz_inventoryitems_equip(SELECT * FROM inventoryitems_equip WHERE id = " + id + " )";
    //player.customSqlInsert(sql);
    var sql = "INSERT INTO zz_inventoryitems_equip(id,characterid) VALUES(?,?)";
    player.customSqlInsert(sql, "" + id + "", player.getId());
}
...
function scoreAll(item, postion) {
    ...
    if (item != null) {
        ...
        score += item.getStr() * 10;
        score += item.getLuk() * 10;
        score += item.getDex() * 10;
        score += item.getInt() * 10;
        score += item.getPad() * 20;
        score += item.getMad() * 20;

        score += item.getMaxHp() * 10;//hp
        score += item.getMaxMp() * 10;//mp
        score += item.getCraft() * 10; //手技
        score += item.getSpeed() * 10; //速度
        score += item.getJump() * 10; //跳躍
        score += item.getCUC() * 50; //升級次數
        score += item.getCHUC() * 100; //星星等級
        ...
        score += item.getArc() * 50;
        score += item.getArcLevel() * 100;
        score += item.getAut() * 50;
        score += item.getAutLevel() * 100;
