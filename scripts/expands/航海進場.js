...
let sel = npc.askMenuA("" + text, true);
        ...
        if (player.hasItem(2633609, 1)) {
            ...
            npc.broadcastPlayerNotice(7, "[航海爭略]  玩家 ★★★ " + player.getName() + " ★★★ 重置" + BossName + "副本");
            npc.say("重置成功");
        } else {
            npc.say("你重置物品不足");
        }
	...
	if (warn) {
		npc.broadcastPlayerNotice(1, "[航海爭略] : " + player.getName() + " 隊伍人數：" + party.getMembersCount() + " 進入了 [" + BossName + "]副本。");
	}
}
