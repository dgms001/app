...
if (player.getChannel() > 5) {
    ...
    sss += "進場時間為0,8,10,12,16,20,22整點的0~5分! 1-5分流均可入場\r\n";
    ...
    if (true) {
        ...
        if (false) {
            ...
            if ((hour == 0 || hour == 8 || hour == 10 || hour == 12 || hour == 16 || hour == 20 || hour == 22) && (minute >= 0 && minute <= 5)) {
                if (map.getId() != 993062100) {
                    if (map.getId() != 993000200) {
...
switch (selection) {
                    ...
                    if (event == null) {
                        if (!player.hasMesos(5000000)) {
                            ...
                            if (event == null) {
                                ...
                                npc.broadcastPlayerNotice(13, "【世界BOSS-烈焰戰狼[普通]】 : 玩家【" + player.getName() + "】已開啟" + player.getChannel() + "分流等待室,其餘玩家請盡速進場!");
                            }
                        }
                    } else {
                ...
                if (event == null) {
                    ...
                    if (canJoin == "1") {
                        if (members.length < 50) {
                            ...
                            members.push(player);
                            npc.broadcastPlayerNotice(13, "【世界BOSS-烈焰戰狼[普通]】 : 玩家【" + player.getName() + "】已進入" + player.getChannel() + "分流等待室,其餘玩家請盡速進場!");

                            player.changeMap(993062100, 0);
                            // npc.say("BOSS遠征開始啦！", 2060102);
                        } else {
                            npc.say("當前已有50人参與，你無法進入了", 2060102);
                        }
                    } else {
    ...
    case 3:
        ...
        sss += "#v4000850# 公式:2*(戰地/3000+2) 當前獎勵:#r" + 2 * (Math.floor(player.getMapleUnionLevel() / 3000) + 2) + "#b個\r\n";
        sss += "#v2633634# 公式:2*(戰地/1000+1) 當前獎勵:#r" + 2 * (Math.floor(player.getMapleUnionLevel() / 1000) + 1) + "#b個\r\n";
        ...
        sss += "#v4000850# 公式:1*(戰地/3000+2) 當前獎勵:#r" + 1 * (Math.floor(player.getMapleUnionLevel() / 3000) + 2) + "#b個\r\n";
        sss += "#v2633634# 公式:1*(戰地/1000+1) 當前獎勵:#r" + 1 * (Math.floor(player.getMapleUnionLevel() / 1000) + 1) + "#b個\r\n";
        //sss += "#v2633336# 公式:1*(戰地/2500+1) 當前獎勵:#r" + 1 * (Math.floor(player.getMapleUnionLevel() / 2500) + 1) + "#b個\r\n";
        sss += "#fs16##r";
        sss += "[傷害超過20%]\r\n";
        sss += "#fs14##b";
        sss += "#v2049323# :30個\r\n";
        ...
        sss += "#v4000850# 公式:1*(戰地/3000+2) 當前獎勵:#r" + 1 * (Math.floor(player.getMapleUnionLevel() / 3000) + 2) + "#b個\r\n";
        sss += "#v2633634# 公式:1*(戰地/2000+1) 當前獎勵:#r" + 1 * (Math.floor(player.getMapleUnionLevel() / 2000) + 1) + "#b個\r\n";
        //sss += "#v2633336# 公式:1*(戰地/5000+1) 當前獎勵:#r" + 1 * (Math.floor(player.getMapleUnionLevel() / 5000) + 1) + "#b個\r\n";
        sss += "#fs16##r";
        sss += "[保底獎勵]\r\n";
        sss += "#fs14##b";
        sss += "#v2049323# :10個\r\n";
