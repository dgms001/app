...
var randList = Array(
	Math.floor(Math.random() * 10 + 1), //str
	Math.floor(Math.random() * 10 + 1), //dex
	Math.floor(Math.random() * 10 + 1), //inte
	Math.floor(Math.random() * 10 + 1), //luk
	Math.floor(Math.random() * 10 + 1), //pad
	Math.floor(Math.random() * 10 + 1), //mad
	Math.floor(Math.random() * 5 + 3), //boss
	Math.floor(Math.random() * 2 + 1), //wushi
	Math.floor(Math.random() * 200 + 1), //hp
	Math.floor(Math.random() * 200 + 1) //mp
...
avail += "#b這裡是埃蘇防具屬性提升中心:#v1152179##v1152179##v1152179##v1152179#\r\n\r\n#k";
avail += "2、提升屬性需要#v" + need + "#一顆，#r如果選擇保留強化屬性需要額外消耗一顆#k 該物品在裝備分解中獲得\r\n";
avail += "#r3、#r注意：裝備不支持增加火花提升的屬性\r\n";
avail += "4、#r注意：提升的時候可以先砸卷後提升\r\n";
avail += "5、#r注意：把要提升的埃蘇裝備放在裝備欄的第一個位置！\r\n#k";
avail += " #L0##b我要強化埃蘇裝備！#l#k\r\n#k";
		...
		if (flag == 1) {
			...
			if (times < 15) {
				...
				if (sel == 1) {
					...
					equip.setTitle("埃蘇強化+" + times);
					player.updateItem(1, equip);
					npc.broadcastGachaponMsgEx("【強化中心】 : 玩家 " + player.getName() + " 強化了埃蘇裝備，獲得了強大的力量！！!", equip);
