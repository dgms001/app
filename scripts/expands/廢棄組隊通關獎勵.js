...
let now = getEventValue("通關廢棄組隊", false);
let max = 3;//最大領獎次數
...
if (party != null && player.getId() == party.getLeader()) {
    ...
    //npc.broadcastPlayerNotice(13, "【玩具101獎勵】:玩家 " + player.getName() + " 帶領隊員通關玩具城！獲得隊長獎勵點券 x1！");
...
if (max > now) {
    ...
    if ((now + 1) % 5 == 0) {
        ...
        if (ran < chance) {
            //player.gainGachaponItem("[玩具101]", 4001714, 1, 1);
        }
    }

    setEventValue("通關廢棄組隊", 1);
    npc.broadcastPlayerNotice(15, "[第一次同行]: " + player.getName() + " 今日已成功通關第一次同行組隊任務" + (now + 1) + "次,獲得大量獎勵！(賬號每日" + max + "次)");
} else {
