...
/*------------------------------------------------------------*/
itemId = 4009454;
txt = "\t\t\t  " + SR3 + "#fs32##fc0xFF00caf2##e《楓點任務》" + SR3 + "\r\n";
...
txt += "#e#fs17#";
txt += "#b任務屬性:[帳號限定]\t需求材料:[#v4009454#*1000]\r\n";
txt += "#fc0xFFFFF300##L1#" + GC1 + "楓點任務[100等][每日2次][獲得#v4032053#*500][20經驗]#l \r\n";
txt += "#L2#" + GC1 + "楓點任務[200等][每日2次][獲得#v4032053#*500][30經驗]#l \r\n";
txt += "#L3#" + GC1 + "楓點任務[250等][每日1次][獲得#v4032053#*500][40經驗]#l \r\n";
txt += "#L4#" + GC1 + "楓點任務[285等][每日1次][獲得#v4032053#*500][50經驗]#l \r\n";
if (player.isGm()) {
...
switch (selection) {
	...
	case 1:
		if (player.getLevel() < 100) {
		...
		} else if (player.getEventValue("楓點任務1") >= 2) {
			...
			player.addEventValue("楓點任務1", 1, 1);
			addvalue(20);
			npc.say("#fs18##b#e獲得#g#v4032053##z4032053# #b* 500 個");
			player.runScript("楓點任務");
		}
		break;
	case 2:
		if (player.getLevel() < 200) {
			npc.say("#fs18##b#e玩家等級不足200等");
		} else if (player.getEventValue("楓點任務2") >= 2) {
			npc.say("#fs18##b#e今日次數已用完");
		} else if (!player.hasItem(itemId, 1000)) {
			npc.say("#fs18##b#e玩家持有的道具不足");
		} else {
			player.loseItem(itemId, 1000);
			player.gainItem(4032053, 500);
			player.addEventValue("楓點任務2", 1, 1);
			addvalue(30);
			npc.say("#fs18##b#e獲得#g#v4032053##z4032053# #b* 500 個");
			player.runScript("楓點任務");
		}
	...
	case 3:
		if (player.getLevel() < 250) {
		...
		} else if (player.getEventValue("楓點任務3") >= 1) {
			npc.say("#fs18##b#e今日次數已用完");
		} else if (!player.hasItem(itemId, 1000)) {
			npc.say("#fs18##b#e玩家持有的道具不足");
		} else {
			player.loseItem(itemId, 1000);
			player.gainItem(4032053, 500);
			player.addEventValue("楓點任務3", 1, 1);
			addvalue(40);
			npc.say("#fs18##b#e獲得#g#v4032053##z4032053# #b* 500 個");
			player.runScript("楓點任務");
		}
	...
	case 4:
		if (player.getLevel() < 285) {
		...
		} else if (player.getEventValue("楓點任務4") >= 1) {
			npc.say("#fs18##b#e今日次數已用完");
		} else if (!player.hasItem(itemId, 1000)) {
			npc.say("#fs18##b#e玩家持有的道具不足");
		} else {
			player.loseItem(itemId, 1000);
			player.gainItem(4032053, 500);
			player.addEventValue("楓點任務4", 1, 1);
			addvalue(50);
			npc.say("#fs18##b#e獲得#g#v4032053##z4032053# #b* 500 個");
