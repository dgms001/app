...
avail+="#b#fs21#這裡是圖騰屬性提升中心:#v1202160##v1202299#\r\n\r\n#k";
		avail +="#b#fs16#每次提升屬性\r\n[力量#r+5#b＆敏捷#r+5#b＆智力#r+5#b＆運氣#r+5#b＆物攻#r+5#b＆魔攻#r+5#b]\r\n";
		avail += "#b提升屬性需要#r#v" + need + "#*1，#v" + need2 + "#*15，#v" + need3 + "#*5000，#v" + need4 + "#*9999\r\n\r\n";
...
if(selection == 0) {
	...
	if(!player.hasItem(need,1)||!player.hasItem(need2,15)||!player.hasItem(need3,5000)||!player.hasItem(need4,9999)) {
...
//			npc.say("你並沒有#z" + need + "#，所以不能開啟"+player.hasItem(need,1)+player.hasItem(need2,1)+player.hasItem(need3,1000));
			...
			if(times < 20) {
				...
				if(sel == 1) {
					...
					npc.broadcastGachaponMsgEx("【強化中心】 : 玩家 "+player.getName()+" 強化了圖騰系列，獲得了強大的力量！！!", equip);
					
					player.loseItem(need, 1);
					player.loseItem(need2, 15);					
