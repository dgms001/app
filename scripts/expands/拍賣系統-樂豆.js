...
ban();
retur();
if (60 - (timestamp - getAuctionsTmie()) / 1000 > 0 && 60 - (timestamp - getAuctionsTmie()) / 1000 < 10) {
...
if ((timestamp - getAuctionsTmie()) / 1000 < 60) {
    txt += "#fs11##r";
    txt += "\t\t\t\t正在競標的商品為:#v" + whethershelves() + "#\r\n\t\t";
    txt += "" + 60 - (timestamp - getAuctionsTmie()) / 1000 + " 秒後無人出價競拍結束\r\n"
    txt += "\t\t\t#b當前價格:" + getmoney() + " 競拍玩家:" + getname() + "\r\n"
    txt += "\t\t\t#b距離你下次競標還有[" + TOP + "]秒\r\n"
    if (TOP < 0) {
        txt += "\t\t\t#r#L2#" + Icon[103][1] + "我要競價[樂豆模式]\r\n";
    }
}
...
if (player.getName() == getname() && (timestamp - getAuctionsTmie()) / 1000 > 60) {
    txt += "#L3#" + Icon[103][1] + "買家領取拍賣物品\r\n";

}
if (player.getEventValue("上架商品記錄") > 0 && (timestamp - getAuctionsTmie()) / 1000 > 60) {
...
if (selection == 0) {
    if (!player.hasItem(4310086, 1)) {
        ...
        dijia = npc.askNumber("輸入低價：", 1, 1, 999999);
        ...
        npc.broadcastWeatherEffectNotice(20, "[競拍系統] 玩家 " + player.getName() + " 上架了自由轉職硬幣,底價:" + dijia + "樂豆!!", 100000);
    } else {
        npc.say("已經有人上架了物品哦");
    }
} else if (selection == 1) {
    if (!player.hasItem(4310237, 1)) {
        ...
        player.addEventValue("上架商品記錄", 1, 1000);
        npc.broadcastWeatherEffectNotice(20, "[競拍系統] 玩家 " + player.getName() + " 上架了NOVA硬幣,底價:" + dijia + "樂豆!!", 100000);
    } else {
        npc.say("已經有人上架了物品哦");
    }
} else if (selection == 2) {
    var timestamp = new Date().getTime();
    var time = 1000 * 60 * 60;
    paimaijia = npc.askNumber("#b請加價:\r\n#r[加價不是現價,你輸入多少就是 現在的價格+你輸入的價格]：", 100, 100, 9999);
    if ((timestamp - getAuctionsTmie()) / 1000 > 60) {
        npc.say("本次競拍已經結束,請等待買賣雙方領取完畢即可下次拍賣");
    } else if (player.getEventValue("上架商品記錄") > 0) {
        npc.say("賣家不可參與競價");
    } else if (player.getCashShopCurrency(1) < (getmoney() + paimaijia)) {
        npc.say("#fs16##b#e樂豆不足" + (getmoney() + paimaijia) + "，無法競價");
    } else if (getbid() == 0) { //競拍
        insbid(1, paimaijia, timestamp, 0); //上傳
        npc.broadcastPlayerNotice(37, "玩家" + player.getName() + "發出競價 當前價格:" + getmoney() + "樂豆!");
    ...
    if (player.getCashShopCurrency(1) < getmoney()) {
        player.addEventValue("惡意拍賣記錄", 1, 3000)
        npc.say("#fs16##b#e樂豆不足" + getmoney() + "，無法領取,惡意拍賣記錄" + player.getEventValue("惡意拍賣記錄") + "次 三次後永久不可參與競拍");
    } else if (getod() == 0) { //if == 1 才可以領
        npc.say("#fs16##b#e賣家還未領取,請稍等!");
    } else if (player.getName() == getname()) {
        npc.broadcastPlayerNotice(37, "玩家" + player.getName() + "花費" + getmoney() + "樂豆 從一億人競標中獲得了競拍物品!");
        player.modifyCashShopCurrency(1, - getmoney());
...
} else if (selection == 4) { //賣出拍賣品 獲得樂豆
    if (player.getEventValue("上架商品記錄") > 0) {
        npc.broadcastPlayerNotice(37, "玩家" + player.getName() + "在競標中賣出了競標物品,價格為:" + getmoney() + "樂豆,恭喜這個B!");
        player.modifyCashShopCurrency(1, getmoney());
        player.addEventValue("拍賣商品獲得樂豆", getmoney(), 999);
        player.resetEventValue("上架商品記錄");
        upod(1, 1);
        npc.say("獲得" + getmoney() + " 樂豆,恭喜!");
        //公告
    } else {
        npc.say("你不應該點開這個");
