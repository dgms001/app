...
var selStr = "#fs21##e#r幫你升級到#v1052720##z1052720#\r\n";
selStr += "#fs16##e#r只要你給我以下材料:#l\r\n";
for(var i = 0; i < needList.length; i++) {
	selStr += "\r\n#v" + needList[i][0] + "#x" + needList[i][1];
}
//selStr += "#e#r,我可以幫您升級到#v1052719##z1052719#\r\n\r\n";
selStr += "\r\n請把#v1052719#放在裝備欄的第一格。";
let selection = npc.askYesNo(selStr);
if(selection == 1) {
	var item = player.getInventorySlot(1, 1);
	if(item == null) {
		npc.say("請把#v1052719#放在裝備欄的第一格。");
	} else if(item.getDataId() != 1052719) {
		npc.say("請把#v1052719#放在裝備欄的第一格。");
	} else {
		var flag = 1;
		for(var i = 0; i < needList.length; i++) {
		...
		if(flag == 1) {
			...
			var toDrop = player.makeItemWithId(1052720);
			...
			npc.broadcastPlayerNotice(7, "[滅龍製作] : 恭喜 " + player.getName() + " 製作出   ★★★正式鬥劍士騎士盔甲★★★" );
			npc.say("成功製作。");
