...
let itemIdS = 5062026;//獲得

txt = "\t\t\t#fs17##r五一活動#v" + itemIdS + "#\r\n";
txt += "#e#fs11#";
txt += "#b任務屬性:[帳號限定]\t每次獲得:[#v" + itemIdS + "#*10]\r\n";
if (player.getEventValue("五一活動草原1") < 5) {
	txt += "#L1#" + GC1 + "五一活動草原1[210等][5次][消耗#v" + itemId + "#" + itemIdC[0] + "]#l \r\n";
} if (player.getEventValue("五一活動草原2") < 5) {
	txt += "#L2#" + GC1 + "五一活動草原2[230等][5次][消耗#v" + itemId + "#" + itemIdC[1] + "]#l \r\n";
} if (player.getEventValue("五一活動草原3") < 5) {
	txt += "#L3#" + GC1 + "五一活動草原3[250等][5次][消耗#v" + itemId + "#" + itemIdC[2] + "]#l \r\n";
} if (player.getEventValue("五一活動草原4") < 5) {
	txt += "#L4#" + GC1 + "五一活動草原4[260等][5次][消耗#v" + itemId + "#" + itemIdC[3] + "]#l \r\n";
}
if (player.isGm()) {
...
switch (selection) {
	...
	case 1:
		if (player.getLevel() < 210) {
		...
		} else if (player.getEventValue("五一活動草原1") >= 5) {
			...
			player.addEventValue("五一活動草原1", 1, 99);
	...
	case 2:
		if (player.getLevel() < 230) {
		...
		} else if (player.getEventValue("五一活動草原2") >= 5) {
			npc.say("#fs18##b#e活動期間次數已滿");
			...
			player.gainItem(itemIdS, 10);
			player.addEventValue("五一活動草原2", 1, 99);
			npc.say("#fs18##b#e獲得#g#v" + itemIdS + "##z" + itemIdS + "## #b* 2500 個");
			addvalue(20);
		}
		break;
	case 3:
		if (player.getLevel() < 250) {
		...
		} else if (player.getEventValue("五一活動草原3") >= 5) {
			npc.say("#fs18##b#e活動期間次數已滿");
			...
			player.gainItem(itemIdS, 10);
			player.addEventValue("五一活動草原3", 1, 99);
			npc.say("#fs18##b#e獲得#g#v" + itemIdS + "##z" + itemIdS + "## #b* 2500 個");
			addvalue(30);
		}
		break;
	case 4:
		if (player.getLevel() < 260) {
		...
		} else if (player.getEventValue("五一活動草原4") >= 5) {
			npc.say("#fs18##b#e活動期間次數已滿");
			...
			player.gainItem(itemIdS, 10);
			player.addEventValue("五一活動草原4", 1, 99);
			npc.say("#fs18##b#e獲得#g#v" + itemIdS + "##z" + itemIdS + "## #b* 2500 個");
			addvalue(40);
		}
		break;
