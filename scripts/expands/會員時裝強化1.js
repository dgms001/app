...
if (equip1 == null) {
    npc.say("#fs14##b請在裝備欄第一個位置放上裝備！");
} else if (player.getEventValue("會員積分") < 20) {
    npc.say("#fs14##b會員積分不足20點!!");
} else {
                ...
                if (str < 可強化次數) {
                    ...
                    if (sel == 1) {
                        ...
                        if (SS <= 699) {
                            ...
                            npc.broadcastGachaponMsgEx("【會員時裝強化】:玩家[" + player.getName() + "]的時裝強化成功至[" + str2 + "]", equip);
                            for (var key in ItemSl) {
                                player.loseItem(ItemSl[key][0], ItemSl[key][1]);
                            }

                            player.addEventValue("會員積分", -20, 1000);
                            ...
                            npc.broadcastGachaponMsgEx("【會員時裝強化】:玩家[" + player.getName() + "]的時裝強化失敗,階級維持:[" + str2 + "]", equip);
                        }
                        //npc.say("#fs14##b"+SS+"");
