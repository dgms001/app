...
let itemIdS = 4001551;//獲得

txt = "\t\t\t#fs17##r五一活動#v" + itemIdS + "#\r\n";
txt += "#e#fs11#";
txt += "#b任務屬性:[帳號限定]\t每次獲得:[#v" + itemIdS + "#*5]\r\n";
if (player.getEventValue("五一活動閃炫方塊1") < 5) {
	txt += "#L1#" + GC1 + "五一活動1[210等][5次][消耗#v" + itemId + "#" + itemIdC[0] + "]#l \r\n";
} if (player.getEventValue("五一活動閃炫方塊2") < 5) {
	txt += "#L2#" + GC1 + "五一活動2[230等][5次][消耗#v" + itemId + "#" + itemIdC[1] + "]#l \r\n";
} if (player.getEventValue("五一活動閃炫方塊3") < 5) {
	txt += "#L3#" + GC1 + "五一活動3[250等][5次][消耗#v" + itemId + "#" + itemIdC[2] + "]#l \r\n";
} if (player.getEventValue("五一活動閃炫方塊4") < 5) {
...
switch (selection) {
	...
	case 1:
		...
		} else if (player.getEventValue("五一活動閃炫方塊1") >= 5) {
			npc.say("#fs18##b#e活動期間次數已滿");
		} else if (!player.hasItem(itemId, itemIdC[0])) {
			npc.say("#fs18##b#e玩家持有的道具不足");
		} else {
			player.loseItem(itemId, itemIdC[0]);
			player.gainItem(itemIdS, 5);
			player.addEventValue("五一活動閃炫方塊1", 1, 99);
			npc.say("#fs18##b#e獲得#g#v" + itemIdS + "##z" + itemIdS + "## #b* 5 個");
			addvalue(10);
		}
		break;
	case 2:
		if (player.getLevel() < 230) {
		...
		} else if (player.getEventValue("五一活動閃炫方塊2") >= 5) {
			npc.say("#fs18##b#e活動期間次數已滿");
			...
			player.gainItem(itemIdS, 5);
			player.addEventValue("五一活動閃炫方塊2", 1, 99);
			npc.say("#fs18##b#e獲得#g#v" + itemIdS + "##z" + itemIdS + "## #b* 5 個");
			addvalue(20);
		}
		break;
	case 3:
		if (player.getLevel() < 250) {
		...
		} else if (player.getEventValue("五一活動閃炫方塊3") >= 5) {
			npc.say("#fs18##b#e活動期間次數已滿");
			...
			player.gainItem(itemIdS, 5);
			player.addEventValue("五一活動閃炫方塊3", 1, 99);
			npc.say("#fs18##b#e獲得#g#v" + itemIdS + "##z" + itemIdS + "## #b* 5 個");
			addvalue(30);
		}
		break;
	case 4:
		if (player.getLevel() < 260) {
		...
		} else if (player.getEventValue("五一活動閃炫方塊4") >= 5) {
			npc.say("#fs18##b#e活動期間次數已滿");
			...
			player.gainItem(itemIdS, 5);
			player.addEventValue("五一活動閃炫方塊4", 1, 99);
			npc.say("#fs18##b#e獲得#g#v" + itemIdS + "##z" + itemIdS + "## #b* 5 個");
			addvalue(40);
		}
		break;
