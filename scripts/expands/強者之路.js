/* 圓夢谷 TMS 237 萬能菜單 */
var status = -1
var txt;
/*------------------------- 表情符號 -------------------------*/
var a30 = "#fEffect/CharacterEff/1022223/6/0#" //跑马灯
var a31 = "#fEffect/CharacterEff/1022223/7/0#" //跑马灯
var a32 = "#fEffect/CharacterEff/1022223/8/0#" //跑马灯
var a38 = "#fItem/Pet/5000213.img/stand0/0#";//藍色惡魔
var f2 = "#fUI/CashShop/CSBeauty/hairColor/Enable/2#"
var f3 = "#fUI/CashShop/CSBeauty/hairColor/Enable/3#"
var f4 = "#fUI/CashShop/CSBeauty/hairColor/Enable/4#"
var f5 = "#fUI/CashShop/CSBeauty/hairColor/Enable/5#"
var a10 = "#fEffect/ItemEff.img/1102919/effect/default/0#" //小黄鸭车0-7
var f1 = "#fUI/CashShop/CSBeauty/hairColor/Enable/0#"
var qq1 = "#fUI/EventShop/adventureShop/GradeBuyer/0#"
var s1 = "#fUI/CashShop/CSBeauty/hairColor/Disable/0#"
var a23 = "#fEffect/ItemEff.img/1005254/effect/default/4#" //蓝色中小星星
txt = "#fs21##r#e你好！這裡是強者之路\r\n#fs13#";

txt += "#fs16##L1##b" + a23 + "徽章強化#l#L2#" + a23 + "能源強化#l#L3#" + a23 + "副手強化#l#L4#" + a23 + "機器人強化#l\r\n";
txt += "#fs16##L5##b" + a23 + "航海武器#l#L9#" + a23 + "航海防具#l#L7#" + a23 + "漆黑強化#l#L8#" + a23 + "培羅德強化#l\r\n";
txt += "#fs16##L6##b" + a23 + "神秘武器#l#L10#" + a23 + "神秘防具#l#L11#" + a23 + "贊助強化#l#L16#" + a23 + "寶石鑲嵌#l\r\n";//#L12#" + a23 + "勛章強化#l

txt += "#fs16##L17##b" + a23 + "勛章鑲嵌#l\r\n";
txt += "\t#fs16##L13##b" + a23 + "蘋果強化#l#L15##b" + a23 + "創世解封#l#L14##b" + a23 + "永恆強化#l\r\n\r\n";//#L14##b" + a23 + "永恆強化#l#L15##b" + a23 + "黎明強化#l

let selection = npc.askMenuA(txt, true);

switch (selection) {
    case 1:
        player.runScript("徽章强化");
        break;
    case 2:
        player.runScript("能源強化");
        //npc.sendShop(1011001);
        break;
    case 3:
        player.runScript("副手強化");
        break;
    case 4:
        player.runScript("機器人強化");
        break;
    case 5:
        player.runScript("航海武器");
        break;
    case 6:
        player.runScript("神秘武器");
        break;
    case 7:
        player.runScript("漆黑強化");
        break;
    case 8:
        player.runScript("培羅德強化");
        break;
    case 9:
        player.runScript("航海防具");
        break;
    case 10:
        player.runScript("神秘防具");
        break;
    case 11:
        player.runScript("贊助強化");
        break;
    case 12:
        player.runScript("勛章強化");
        break;
    case 13:
        player.runScript("蘋果強化");
        break;
    case 14:
        player.runScript("永恆");
        break;
    case 15:
        player.runScript("創世");
        break;
    case 16:
        player.runScript("NV幣強化");
        break;
    case 17:
        player.runScript("勛章鑲嵌");
        break;
    case 50:
        player.runScript("本服菜單");
        break;
}
