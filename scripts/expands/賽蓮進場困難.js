...
let maxLevel = [300];
let maxenter = [1]
let BossName = ["受選的賽蓮[困難]"];
let PQLog = ["受選的賽蓮[困難]"]; //Seren
let event = ["boss_seren_hard"];
let onlyOne = true;
...
if (player.getChannel() <= 5) {
    ...
    if (player.getEventValue("賽蓮全員獎勵") > 0) {
        text += "\t #L3#購買核心/日/次[20000楓葉]#l";
    }
}
let sel = npc.askMenu("" + text, true);
if (sel == 3) {
    if (player.getEventValue("賽蓮核心每日購買次數") == 0 && player.hasItem(4032053, 20000)) {
        if (player.gainVSkill(10000031)) {
