...
var selStr = "#fs21##e#r幫你升級到#v1052719##z1052719#\r\n";
...
selStr += "\r\n請把#v1052718#放在裝備欄的第一格。";
let selection = npc.askYesNo(selStr);
if(selection == 1) {
	var item = player.getInventorySlot(1, 1);
	if(item == null) {
		npc.say("請把#v1052718#放在裝備欄的第一格。");
	} else if(item.getDataId() != 1052718) {
		npc.say("請把#v1052718#放在裝備欄的第一格。");
	} else {
		var flag = 1;
		for(var i = 0; i < needList.length; i++) {
		...
		if(flag == 1) {
			...
			var toDrop = player.makeItemWithId(1052719);
			...
			npc.broadcastPlayerNotice(7, "[滅龍製作] : 恭喜 " + player.getName() + " 製作出   ★★★見習鬥劍士騎士盔甲★★★" );
			npc.say("成功製作。");
