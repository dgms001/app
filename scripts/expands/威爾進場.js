...
if(player.getEventLog("威爾普通全員獎勵") > 0){
//if (player.isGm()) {
    text += "\r\n\t\t\t  #L3#購買核心/日/次[10000楓晶]#l";
}

let sel = npc.askMenuA("" + text, true);
if (sel == 3) {
    if (player.getEventLog("威爾核心每日購買次數") == 0 && player.hasItem(4033078, 10000)) {
        if (player.gainVSkill(10000024)) {
            player.addEventLog("威爾核心每日購買次數", 1, 1);
            player.loseItem(4033078, 10000);
        ...
        npc.say("今日已經購買過了\r\n或者你風晶不足!");
			      ...
			      if (sel == 6) {
					  var xuanze = npc.askYesNo("#fs16##e#b重置普通點擊[#r是#b],重置困難點擊[#r否#b]");
					   var bossevent = Array("boss_papulatus_normal","boss_papulatus_chaos");
