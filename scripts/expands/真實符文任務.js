...
/*------------------------------------------------------------*/
itemId = 4009454;
txt = "\t\t " + SR3 + "#fs32##fc0xFF00caf2##e《真實符文任務》" + SR3 + "\r\n";
...
txt += "#fc0xFFFFF300#";
txt += "";
txt += "#L1#" + GC1 + "符文任務[260等][每日1次][獲得#v2633336#*1][20經驗]#l \r\n";
txt += "#L2#" + GC1 + "符文任務[265等][每日1次][獲得#v2633336#*2][30經驗]#l \r\n";
if (player.isGm()) {
...
switch (selection) {
	...
	case 1:
		if (player.getLevel() < 260) {
		...
		} else if (player.getEventValue("真實ARC任務1") >= 1) {
			...
			player.addEventValue("真實ARC任務1", 1, 1);
			addvalue(20);
			npc.say("#fs18##b#e獲得#g#v2633336##z2633336# #b* 1 個");
			player.runScript("真實符文任務");
		}
		break;
	case 2:
		if (player.getLevel() < 265) {
			npc.say("#fs18##b#e玩家等級不足265等");
		} else if (player.getEventValue("真實ARC任務2") >= 1) {
			npc.say("#fs18##b#e今日次數已用完");
		} else if (!player.hasItem(itemId, 5000)) {
			npc.say("#fs18##b#e玩家持有的道具不足");
		} else {
			...
			//player.gainItem(4310011, 2);//聖誕物品
			player.addEventValue("真實ARC任務2", 1, 1);
			addvalue(30);
			npc.say("#fs18##b#e獲得#g#v2633336##z2633336# #b* 2 個");
